...
# allowDangerousTypes: PLAN.md §2.3 uses float64 fractions/multipliers
# (errorRateFraction: 0.05). CRD OpenAPI discourages floats across
# languages; this flag keeps the locked YAML instead of renaming fields.
...
INTEGRATION_CONTEXT ?= kind-cascade-operator

.PHONY: test-integration
test-integration: ## Run Kind integration tests against the dev cluster (//go:build integration).
...
KIND_CLUSTER ?= cascade-operator-test-e2e
...
.PHONY: verify-fmt
verify-fmt: ## Fail if any Go file is not gofmt'd.
	@unformatted="$$(gofmt -l .)"; \
...
verify-generate: generate manifests ## Fail if generated deepcopy/CRD/RBAC drift from git.
	@git diff --exit-code -- api/v1alpha1/zz_generated.deepcopy.go config/crd config/rbac/role.yaml
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name cascade-operator-builder
	$(CONTAINER_TOOL) buildx use cascade-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm cascade-operator-builder
	rm Dockerfile.cross
