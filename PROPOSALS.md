...
**Proposed by:** Claude
**Date:** 2026-08-30
**Affects:** 2.6 Mitigation (retry-storm primary and secondary); `internal/mitigation/retries.go`'s `ApplyRetryStormTrip`, `internal/mitigation/retry_connpool.go`'s `ApplyRetryStormConnectionPoolTrip`

**Current state:** Both retry storm patch functions set a trip value of exactly `0` on a plain `int32` proto field — `VirtualService.HTTPRoute.Retries.Attempts` (primary) and `DestinationRule.ConnectionPoolSettings_HTTPSettings.MaxRetries` (secondary, this session's own slice). Neither field is a nullable wrapper type (unlike `outlierDetection`'s `Consecutive_5XxErrors`, a `*wrapperspb.UInt32Value`) — both carry `json:"...,omitempty"` on a bare scalar. Both patch functions write through the typed `controller-runtime` client's `Update()` call on the full object.

**Proposed change:** Change the write path for these two specific fields — and only these two — from the typed struct's `Update()` to a raw JSON/merge-patch: build the patch as explicit bytes or an `unstructured.Unstructured`/raw-map value so the JSON literally contains `"attempts":0` / `"maxRetries":0`, and apply it via `client.Patch()` (merge or JSON patch) instead of `client.Update()` on the typed struct. Every other patch function in the mitigation package (`outlierDetection`, `timeout`, fan-out's connectionPool fields) is unaffected and stays on typed `Update()` — this is not a blanket change to the write mechanism, only a fix scoped to the two fields whose intended trip value happens to equal their type's JSON zero value.

**Why:** Verified conclusively, not theorized — full investigation in `docs/worklog/2026-08-30-retry-storm-zero-value-serialization-bug.md`. Vendored `istio.io/api` source confirms both fields are plain `int32` with `omitempty`; a standalone marshal test on the bare structs produces `{}`; the same test on the full `client-go` wrapper types produces the same result; running the actual unmodified production function (`ApplyRetryStormConnectionPoolTrip`) against a real live Kind cluster and then reading the **raw stored JSON** via `kubectl get -o json` (not the Go struct read-back, which is ambiguous between "explicit 0" and "absent") shows `"http":{}` — no `maxRetries` key at all. The `Update()` call itself reports success throughout; the value simply never reaches the API server. This means Envoy has likely never actually enforced either mitigation at zero retries — it falls back to its own default (`max_retries: 3`) and Istio's implicit default (`attempts: 2`), the exact opposite of what retry storm's mitigation is supposed to achieve. Every existing test passes because the project's fake-client unit tests never round-trip through real JSON marshal/unmarshal, and every prior live check verified `Update()` success and CR status transitions, not Envoy's actual rendered proxy config.

Rejected alternative: bumping the trip value from `0` to `1` so it serializes correctly under `omitempty` (as considered and rejected for a different field/reason in the retry-storm-webhook proposal above). Rejected here for the same reason it stayed rejected there — `attempts: 1`/`maxRetries: 1` is a real, already-reasoned-through weakening of the mitigation's strength (a single retry still allowed can itself sustain amplification at or near a low `retryStormMultiplier` threshold), and a correct fix exists that doesn't require accepting that trade-off. Changing the number to dodge a serialization bug would be fixing the symptom, not the bug.

**Impact if approved:** Touches exactly two functions: `ApplyRetryStormTrip` (`internal/mitigation/retries.go`) and `ApplyRetryStormConnectionPoolTrip` (`internal/mitigation/retry_connpool.go`). Both need their `Attempts`/`MaxRetries` write switched to a patch that preserves an explicit `0` in the outgoing JSON. Restoration paths (`retry_restore.go`) are unaffected in mechanism — they write non-zero values, which already serialize correctly — but should get a live re-check once the trip-side fix lands, to confirm the full trip→restore cycle now round-trips a real `0` and back. Existing fake-client unit tests for both functions stay valid for their in-memory-struct assertions but do not, by themselves, prove this fix works — live re-verification against the real cluster (checking the raw stored JSON, and ideally Envoy's actual rendered config via `istioctl proxy-config` or the Envoy admin API, not just the Kubernetes object) is required before this can be marked done in PLAN.md.

**Resolved by Claude, 2026-08-30: APPROVED — patch-based write for `Attempts`/`MaxRetries`, not a trip-value change.** This is my own proposal, filed and resolved in the same pass given the severity (it revisits two previously-approved slices' actual runtime correctness) and the user's explicit go-ahead to prioritize it. PLAN.md §2.6 gets a note recording this as a known-and-now-being-fixed gap rather than silently amending the matrix, since the mitigation *strategy* isn't changing, only how the zero value is transmitted. Implementation — the patch-based rewrite of both functions, plus live re-verification against the raw stored object and, ideally, Envoy's rendered config — is the next Cursor slice, ahead of any other unfinished work.
